...
namespace association {
    ...
    extern double inlier_ratio;
    extern int    max_iterations;
    // MDRANSAC
    extern int    max_gn_iterations;
    extern double md_threshold;
}
...
public:
    MotionDistortedRansac(Eigen::MatrixXd p1, Eigen::MatrixXd p2, std::vector<int64_t> t1, std::vector<int64_t> t2,
            ...
            : tolerance(tolerance_), inlier_ratio(inlier_ratio_), iterations(iterations_) {
        ...
        for (uint i          = 0; i < p1bar.cols(); ++i) {
            ...
            delta_ts[i] = double(delta_t) / denominator;
