...
double cen2019features(cv::Mat fft_data, int max_points, int min_range, Eigen::MatrixXd &targets,
    ...
    std::vector<std::vector<cv::Point2f>> t(fft_data.rows);
    std::vector<cv::Point2f>              t_test(fft_data.rows);
    t_test.clear();
//#pragma omp parallel for
    for (int i = 0; i < fft_data.rows; i++) {
        // Find the continuous marked regions in each azimuth
        ...
        if (!t[i].empty()) {
            ...
            float     max_candidate;
            float     max     = 0;
            for (auto iter    = t[i].begin(); iter != t[i].end(); iter++, index++) {
                max_candidate = h.at<float>(i, iter->y);
                if (index == 0) {
                    max = max_candidate;
                    max_col = iter->y;
                } else if (max_candidate > max) {
                    max = max_candidate;
                    max_col = iter->y;
                }
            }
            t_test.push_back(cv::Point(i, max_col));
        } else {
            t_test.push_back(cv::Point(i, 0));
        }

    }
    targets = Eigen::MatrixXd::Ones(3, t_test.size());
    for (uint k = 0; k < t_test.size(); ++k) {
        targets(0, k) = t_test[k].x;
        targets(1, k) = t_test[k].y;
    }
...
double cen2019descriptors(std::vector<double> azimuths, cv::Size polar_dims, Eigen::MatrixXd polar_points,
                          Eigen::MatrixXd cart_targets, float radar_resolution, float cart_resolution,
...
#pragma omp parallel for
    ...
    cv::Mat   cart_binary;
    polar_to_cartesian_radar_img(azimuths, polar_binary, radar_resolution, cart_resolution, cart_pixel_width, true,
