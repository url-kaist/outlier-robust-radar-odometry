...
void polar_to_cartesian(const Eigen::Vector2d &p_rtheta, Eigen::Vector2d &p_xy) {
    p_xy(0) = p_rtheta(0) * cos(p_rtheta(1));
    p_xy(1) = p_rtheta(0) * sin(p_rtheta(1));
}

void cartesian_to_polar(const Eigen::Vector2d &p_xy, Eigen::Vector2d &p_rtheta) {
    p_rtheta(0) = sqrt(p_xy(0) * p_xy(0) + p_xy(1) * p_xy(1));
    p_rtheta(1) = atan2(p_xy(1), p_xy(0));
}

void cart2polar(const Eigen::MatrixXd &cartesian, Eigen::MatrixXd &polar) {
    polar      = Eigen::MatrixXd::Ones(2, cartesian.cols());
    for (int i = 0; i < cartesian.cols(); i++) {
        polar(0, i) = sqrt(cartesian(0, i) * cartesian(0, i) + cartesian(1, i) * cartesian(1, i));
        polar(1, i) = atan2(cartesian(1, i), cartesian(0, i));
    }
}

void polar2cart(const Eigen::MatrixXd &polar, Eigen::MatrixXd &cartesian) {
    cartesian  = Eigen::MatrixXd::Ones(2, polar.cols());
    for (int i = 0; i < polar.cols(); i++) {
        cartesian(0, i) = polar(0, i) * cos(polar(1, i));
        cartesian(1, i) = polar(0, i) * sin(polar(1, i));
    }
}

void dopplerCorrection(const Eigen::VectorXd &wbar, const double beta, Eigen::MatrixXd &p2d) {
    // To take stop motion into account.
    double   v = isnan(wbar(0, 0)) ? 0 : fabs(wbar(0, 0));
...
std::vector<float> splitLine(std::string input, char delimiter, int64 &timestamp) {
...
void load_radar(std::string path, std::vector<int64_t> &timestamps, std::vector<double> &azimuths,
    ...
    int range_bins = [&]() -> int {
        if (navtech_version == CIR204) return 3360;
        else return 3768; // should be
    }();
...
#pragma omp parallel
    for (int i = 0; i < N; ++i) {
        ...
        //if(i%200==0) std::cout<<i<<"th azimuth : "<<azimuths[i]<<std::endl;
    }
}
...
void load_velodyne(std::string path, std::vector<int64_t> &timestamps, std::vector<double> &azimuths,
    ...
    std::vector<double> elevations                 = {-0.1862, -0.1628, -0.1396, -0.1164, -0.0930, -0.0698, -0.0466,
                                                      -0.0232, 0.,
                                                      0.0232, 0.0466, 0.0698, 0.0930, 0.1164, 0.1396, 0.1628, 0.1862,
                                                      0.2094, 0.2327, 0.2560, 0.2793, 0.3025, 0.3259,
                                                      0.3491, 0.3723, 0.3957, 0.4189, 0.4421, 0.4655, 0.4887, 0.5119,
                                                      0.5353};
...
void polar_to_cartesian_radar_img(std::vector<double> &azimuths, cv::Mat &fft_data, float radar_resolution,
...
void polar_to_cartesian_points(std::vector<double> azimuths, std::vector<int64_t> times, Eigen::MatrixXd polar_points,
                               float radar_resolution, Eigen::MatrixXd &cart_points, Eigen::MatrixXd &polar_points_out,
                               std::vector<int64_t> &point_times) {
    polar_points_out = Eigen::MatrixXd::Ones(2, polar_points.cols());
    cart_points      = polar_points; // polar_points: 3xN (see the function cen2019features():  targets = Eigen::MatrixXd::Ones(3, size);)
    point_times      = std::vector<int64_t>(polar_points.cols());
    for (uint i      = 0; i < polar_points.cols(); ++i) {
        double azimuth = azimuths[polar_points(0, i)];
        double r       = 0;
        if (polar_points(1, i) == 0) r = 0;
        else r = polar_points(1, i) * radar_resolution + radar_resolution / 2;
        polar_points_out(0, i) = r;
        polar_points_out(1, i) = azimuth;
        cart_points(0, i)      = r * cos(azimuth); // meter
        cart_points(1, i)      = r * sin(azimuth); // meter
        point_times[i] = times[polar_points(0, i)];
    }
...
void convert_to_bev_jump(Eigen::MatrixXd &cart_points, float cart_resolution, int cart_pixel_width, int patch_size,
                         std::vector<cv::KeyPoint> &bev_points, std::vector<int64_t> &point_times) {
    float cart_min_range = (cart_pixel_width / 2) * cart_resolution;
    if (cart_pixel_width % 2 == 0)
        cart_min_range = (cart_pixel_width / 2 - 0.5) * cart_resolution;
    ...
    int       j = 0;
    for (uint i = 0; i < cart_points.cols(); ++i) {
        double u = (cart_min_range + cart_points(2, i)) / cart_resolution;
        double v = (cart_min_range - cart_points(3, i)) / cart_resolution;
        if (0 < u - patch_size && u + patch_size < cart_pixel_width && 0 < v - patch_size &&
            v + patch_size < cart_pixel_width)
            bev_points.push_back(cv::KeyPoint(u, v, patch_size));
        else {
            cart_points(0, i) = 0;
            bev_points.push_back(cv::KeyPoint(0, 0, patch_size));
        }
    }
}
...
void draw_points(cv::Mat cart_img, Eigen::MatrixXd cart_targets, float cart_resolution,
                 int cart_pixel_width, cv::Mat &vis, std::vector<uint> color) {
    std::vector<cv::Point2f> bev_points;
...
void draw_points(const cv::Mat &cart_img, const std::vector<cv::KeyPoint> &bev_points,
...
void draw_matches_concat(cv::Mat &img_concat, std::vector<cv::KeyPoint> kp1, std::vector<cv::KeyPoint> kp2,
                         std::vector<bool> is_inliers, int x_shift, int radius) {

    assert(kp1.size() == kp2.size() && kp1.size() == is_inliers.size() && kp2.size() == is_inliers.size());
    int       N = is_inliers.size();
    for (uint i = 0; i < N; ++i) {
        cv::Point2f p1 = kp1[i].pt;
        cv::Point2f p2 = kp2[i].pt;
        p2.x += x_shift;
        int  rand_r   = rand() % 256;
        int  rand_g   = rand() % 256;
        int  rand_b   = rand() % 256;
        auto pt_color = cv::Scalar(rand_b, rand_g, rand_r);
        cv::circle(img_concat, p2, radius, pt_color, -1);
        cv::circle(img_concat, p1, radius, pt_color, -1);
    }
    for (uint i = 0; i < N; ++i) {
        cv::Point2f p1 = kp1[i].pt;
        cv::Point2f p2 = kp2[i].pt;
        p2.x += x_shift;
        if (is_inliers[i]) {
            cv::line(img_concat, p2, p1, cv::Scalar(0, 255, 0), 2);
        } else {
            cv::line(img_concat, p2, p1, cv::Scalar(0, 0, 255), 2);
        }
    }
}
...
int validateArgs(const int argc, const char *argv[], std::string &root, std::string &seq, std::string &app) {
    ...
    for (int i = 1; i <= argc - 2; i += 2) {
        ...
        if (opt == "--root") {
            ...
            std::cerr << "Unknown option " << opt << ", usage:";
            usage(argv);
            exit(1);
        }
    }
