...
// RANSAC
double association::ransac_threshold  = 0.35;
//double association::ransac_threshold = 0.35;
double association::inlier_ratio      = 0.90;
int    association::max_iterations    = 100;
// MDRANSAC
int    association::max_gn_iterations = 10;
double association::md_threshold      = pow(association::ransac_threshold, 2);
//}

// RANSAC
//association::ransac_threshold = 0.35;
//association::inlier_ratio = 0.90;
//association::max_iterations = 100;
// MDRANSAC
//association::max_gn_iterations = 10;
//association::md_threshold = pow(associtaion::ransac_threshold, 2);
...
Eigen::VectorXd SE3tose3(Eigen::MatrixXd T) {
    ...
    Eigen::VectorXd rho;
    if (phi != 0.0) {
        J   = I * sin(phi) / phi + (1 - sin(phi) / phi) * abar * abar.transpose() + cross(abar) * (1 - cos(phi)) / phi;
        rho = J.inverse() * T.block(0, 3, 3, 1);
    } else {}
    rho               = T.block(0, 3, 3, 1);
    ...
    xi(0, 0)             = isnan(xi(0, 0)) ? 0 : xi(0, 0);
    xi(1, 0)             = isnan(xi(1, 0)) ? 0 : xi(1, 0);
    xi(2, 0)             = isnan(xi(2, 0)) ? 0 : xi(2, 0);
    return xi;
}
...
void MotionDistortedRansac::getTransform(double delta_t, Eigen::MatrixXd &Tf) {
    std::cout << "[getTransform] t: " << delta_t << std::endl;
    std::cout << "[getTransform] w_best: " << w_best.transpose() << std::endl;
